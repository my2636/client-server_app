...
public class Server {
    ...
    static String getAction(String clientAnswer) {
        ...
        switch (clientAnswer) {
            case "1":
                serverAnswer = catCount == 0 ? "Коты закончились.. Сервер ждет своего хранителя. 2. Принять нового кота" : "Вот кот - он хранитель сервера. Какие коты еще интересуют? 1. Показать кота" +
                        " 2. Принять нового кота 3. Отдать своего кота";
                break;
            case "2":
                catCount++;
                serverAnswer = catCount == 1 ? "Давай кота. Он будет хранителем сервера. Сервер под защитой." : "Давай кота. Теперь на сервере " + catCount + " котов, они подружились. Какие коты еще интересуют?" +
                        " 1. Показать кота 2. Принять нового кота 3. Отдать своего кота";
                break;
            case "3":
                serverAnswer = catCount == 0 ? "Коты закончились, приходите позже. Сейчас сервер в режиме приема котов." +
                        " 2. Принять нового кота" :
                ...
                break;
            default:
                serverAnswer = "Я могу только: 1. Показать кота " +
                        "2. Принять нового кота 3. Отдать своего кота";
        }
